		...
		if (dlg.DoModal() == IDOK) {
			...
			for (DWORD y = 0; y < height; y++) {
				for (DWORD x = 0; x < width; x++) {
					...
					for (by = -2; by <= 2; by++) {
						for (bx = -2; bx <= 2; bx++) {
							if (bx + x < 0 || bx + x >= width || by + y < 0 || by + y >= height) {
								tempSize--;
								continue;
							}
					...
					tempBlue = secondColor.rgbBlue - tempBlue;
					tempGreen = secondColor.rgbGreen - tempGreen;
					tempRed = secondColor.rgbRed - tempRed;
					if (tempBlue < -30 || tempBlue > 30) {
						diff = 1;
					}
					else if (tempBlue < -30 || tempBlue > 30) {
						diff = 1;
					}
					else if (tempBlue < -30 || tempBlue > 30) {
						diff = 1;
